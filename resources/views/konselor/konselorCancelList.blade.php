...
<nav id="navbar" class=" border-gray-200 fixed w-full z-50 top-0 left-0 h-max bg-[#242424]">
  ...
  <section class="md:max-w-[1200px] mx-auto my-[120px]  flex flex-col md:flex-row  gap-[20px]">
      ...
      <div class="flex justify-between items-center" data-collapse-toggle="filter-cta" aria-controls="filter-cta" aria-expanded="false">
        ...
        <div id="filter-cta" class="hidden md:flex md:flex-col">
         ...
         <form class="flex flex-col gap-[10px] mt-[20px]" action="{{route('indexKonselorCancel')}}" method="GET">
          ...
          <p class="text-[20px]">Status</p>
            <div class="flex items-center mb-4 gap-[10px]">
                <input id="default-checkbox5" name="needAction" type="checkbox" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label for="default-checkbox5" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Need Action</label>
            </div>
            <div class="flex items-center mb-4 gap-[10px]">
                <input id="default-checkbox6" name="actionDone" type="checkbox" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label for="default-checkbox6" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Done</label>
            </div>
...
<div class="relative overflow-x-auto">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            @foreach ($cancel as $res)
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                   {{$res->nama}}
                </th>
                <td class="px-6 py-4">
                    {{$res->tgl_konseling}},  {{$res->jam_konseling}}
                </td>
               
                <td class="px-6 py-4">
                    @if (!$res->isConfirmed && !$res->isRejected)
                        Ongoing
                    @elseif ($res->isConfirmed && !$res->isRejected)
                        Accepted
                        @elseif (!$res->isConfirmed && $res->isRejected)
                       Rejected
                    @endif
                ...
                <td class="px-6 py-4 flex flex-row gap-[10px]">
                    @if (!$res->isConfirmed && !$res->isRejected)
                    <form id="confirmCancel{{ $res->id }}" action="{{ route('confirmCancel', $res->id) }}" method="POST" onsubmit="return confirmCancel({{ $res->id }})">
                        @csrf
                    ...
                    <form id="rejectCancel{{ $res->id }}" action="{{ route('rejectCancel', $res->id) }}" method="POST" onsubmit="return rejectCancel({{ $res->id }})">
                        @csrf
...
<script src="./node_modules/flowbite/dist/flowbite.min.js"></script>
 ...
 <script>
    function confirmCancel(id) {
        return confirm('Apakah Anda yakin ingin menerima pembatalan ini?');
    }
    function rejectCancel(id) {
        return confirm('Apakah Anda yakin ingin menolak pembatalan ini?');
    }
  </script>
 {{-- <script>
